    ...
    def countFitness(self, s: Specimen) -> None:
        ...
        for i in range(len(s.chrsoms["location"].genes)):
            ...
            new = (
                Rect(
                    Point(0, 0),
                    self.rcts[i].getHeight(),
                    self.rcts[i].getWidth(),
                )
                if s.chrsoms["rotation"].genes[i]
                else self.rcts[i]
            )
            # location section
            new = new.cloneOffset(s.chrsoms["location"].genes[i])
            if not self.area.containsRectangle(new) or any(
                map(lambda r: r.collides(new), rcts)
            ):
                s.fitness = 0
                return None
        ...
        for n in range(len(self.rcts)):
            ...
            for i, func in enumerate(
                [r.expandLeft, r.expandRight, r.expandUp, r.expandDown]
            ):
        ...
        for i in range(len(self.rooms)):
            for n in range(len(self.rooms)):
                if self.rooms[n].name in self.rooms[i].neighbours and not rcts[
                    n
                ].neighbours(rcts[i]):
                    s.fitness = 0
                    return None
