...
class TestRoomHandler(unittest.TestCase):
    def testGetLocatedRooms(self):
        rooms = [RoomTemplate("hall", 2, 2, True), RoomTemplate("kitchen", 6, 4, True)]
        ...
        self.assertEqual(result[0].p, locations[0])
        self.assertEqual(result[1].p, locations[1])
    ...
    def testOutOfAreaShouldReturnTrue(self):
        rooms = [RoomTemplate("hall", 2, 2, True), RoomTemplate("kitchen", 12, 4, True)]
        rects = [x.getRectRef() for x in rooms]
